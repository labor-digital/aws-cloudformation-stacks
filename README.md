...
Vollständige, eigenständige Infrastruktur für den Betrieb von ECS-Services: VPC mit öffentlichen und privaten Subnetzen über zwei AZs samt NAT Gateway, ECS-Cluster (EC2 Launch Type), Auto Scaling Group mit ECS Managed Scaling, ALB mit HTTP- und HTTPS-Listener, Aurora-MySQL-Cluster, EFS, AWS Backup mit optionaler Regionskopie, Route 53 DNS Firewall, VPC Flow Logs, CloudWatch-Alarme mit gespeicherten Logs-Insights-Queries, ein SNS-Topic für Alarmbenachrichtigungen, ein `REGIONAL` WAF-WebACL am ALB und ein Overview-Dashboard.
...
- **Das WAF-WebACL (`${AWS::StackName}-waf`, Scope `REGIONAL`) hängt direkt am ALB** und schützt alle Services dahinter. Enthalten: AWS Managed Rules Common, Known Bad Inputs, SQLi, WordPress und PHP, die IP Reputation List, ein Rate-Limit sowie eigene Regeln gegen den WordPress-`batch/v1`-Einstiegspunkt, gegen unauthentifizierte Benutzeranlage über `POST /wp/v2/users` und eine IP-Beschränkung für Admin-Pfade. **Nicht** zu verwechseln mit dem WebACL in `cloudfront-alb-distribution`: das hängt an der Distribution, existiert nur in `us-east-1` und prüft ausschließlich Traffic, der tatsächlich über CloudFront läuft.
...
- **Der Regelsatz wurde am 2026-08-20 bewusst von elf auf sechs Regeln gekürzt** — nur Generisches bleibt: Common, Known Bad Inputs, SQLi und WordPress als Managed Groups plus die beiden Rate-Regeln. Entfernt wurden die PHP-Gruppe (überschneidet sich mit Common und WordPress), die IP-Reputation-Gruppe (wertet immer die Quell-IP aus, sieht bei proxied Sites also nur Cloudflare — und Cloudflare macht Reputation selbst), die Admin-Pfad-IP-Beschränkung (Kunden pflegen ihre Inhalte selbst, und in einem gemischten proxied/direct-Setup funktioniert sie ohnehin nicht) sowie zwei WordPress-Regeln, die aus genau einer Exploit-Kette stammten. Begründungen und Re-Evaluierungskriterien in TASKS.md.
- **Reihenfolge der Promotion:** `KnownBadInputsAction` zuerst — schmal, Payload-spezifisch, und die Virtual-Patching-Schicht zwischen CVE und gepatchtem Image. Dann `SqliRuleSetAction`, dann `WordPressRulesAction`, dann die Rate-Regeln, `CommonRuleSetAction` zuletzt.
- **Login-Endpunkte sind nicht geschützt — eine Lücke, die es schon vor der Kürzung gab.** Die Signatur-Gruppen prüfen den Request-*Inhalt*, und ein Login-POST mit falschem Passwort ist ein völlig regulärer Request; das allgemeine Rate-Limit greift erst bei 2.000 Requests pro IP und 5 Minuten, ein Botnet mit 300 Versuchen/Minute bleibt darunter. Drei Optionen in TASKS.md: ein auf Login-Pfade eingegrenztes Rate-Limit (billig, fängt die laute Mehrheit, **nicht** Credential Stuffing), `AWSManagedRulesATPRuleSet` (genau dafür gebaut, kostet extra pro Request) und — vermutlich am passendsten — ein Metric Filter auf fehlgeschlagene Logins mit Alarm auf dem Cluster-Topic, weil der **Fehlversuche** zählt statt Requests.
...
- **`WafBlockedRequestAlarmThreshold` erst nach dem Wechsel auf `block` setzen.** Im Count-Modus wird nichts blockiert, der Alarm bliebe dauerhaft still und würde Sicherheit vortäuschen.
